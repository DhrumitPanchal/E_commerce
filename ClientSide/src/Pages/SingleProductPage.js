...
function SingleProductPage() {
  ...
  useEffect(() => {
    if (!productData || !user?.likedProducts) {
      return 0;
    }
  ...
  return (
    <>
      <section className="py-[4rem] px-[6rem] max-sm:px-[2rem] h-fit flex justify-between max-sm:flex-col gap-[6rem] ">
        ...
        <div className="flex flex-col w-full bg-white ">
          ...
          <div className="mt-[1.2rem]  mr-[2rem] flex gap-[2rem]">
            ...
            <div
              onClick={() =>
                handleAddToCart(product?._id, quantity, product?.product_price)
              }
      ...
      <h2 className="mt-[1rem] px-[1rem] text-[1.2rem] font-semibold">
        Other products
      </h2>
      <section className="mt-[3rem] mb-[4rem] flex justify-center flex-wrap gap-[3rem]">
        {productData?.map((e) => (
          <ProductCard data={e} />
        ))}
      </section>
